...
let parser;
let stringifiedCartCSV;
let stringifiedAllValidationErrorsCartCSV;
...
describe('All CartParser tests with +65 coverage expected', () => {
	...
	describe('CartParser - unit tests', () => {
		beforeEach(() => {
			stringifiedCartCSV = parser.readFile(cartCSVFilePath);
			stringifiedAllValidationErrorsCartCSV = parser.readFile(allValidationErrorsCSVFilePath);
		});

		test('when CSV data input, readFile returns a string from CSV', () => {
			expect(typeof stringifiedCartCSV).toBe("string")
			expect(stringifiedCartCSV).toContain('Mollis consequat,9.00,3');
		});
		...
		test('when cart CSV data input has header error, validate function return array with proper type', () => {
			const validationsArray = parser.validate(stringifiedAllValidationErrorsCartCSV)
		...
		test('when cart CSV data input has empty cell error, validate function return array with proper type and empty message error', () => {
			const validationsArray = parser.validate(stringifiedAllValidationErrorsCartCSV)
		...
		test('when cart CSV data input has negative number cell error, validate function return array with proper type and negative message error', () => {
			const validationsArray = parser.validate(stringifiedAllValidationErrorsCartCSV)
		...
		test('when an array of items is input, calcTotal function return proper total each price * qty', () => {
			const items1 = [{ price: 3, quantity: 4 }, { price: 5, quantity: 3 }]
			const items2 = [cart.items[0], cart.items[1]]
			const total = parser.calcTotal(items1)
			const total2 = parser.calcTotal(items2)

			expect(total).toEqual(27)
			expect(total2).toEqual(47.64)
		});
	});

	describe('CartParser - integration test', () => {
		// Add your integration test here.
		test('when invalid cart input, parse function throws Validation failed! instance of Error', () => {
			expect(() => parser.parse(allValidationErrorsCSVFilePath)).toThrow(
				new Error('Validation failed!')
			);
		})

		test('when valid cart input, parse function return serialized cart', () => {
